...
**WP06 — World, Combat, and Presentation Polish: implemented, technically verified, visually inspected, and owner-accepted for publication on 2026-08-30**
...
| WP06 | World, combat, and presentation polish | **Implemented and owner-accepted for publication — 2026-08-30** |
| WP07 | Integration, balance, and release acceptance | Unauthorized; not started |
...
WP02 added no District Plan migration, new gameplay content, new stream/schema, save version, or permanent progression. Its implementation task performed no commit, publication, or deployment; on 2026-08-21 the owner separately authorized the evidenced WP02 boundary for a `main` and GitHub Pages browser-playtest release. That historical authorization did not close the comprehension gate or pre-authorize later work; WP03 subsequently began only under the owner's explicit 2026-08-22 task.

## WP03 implementation — District Planning and Cards

**Status: Technical/runtime/visual/platform gate passed on 2026-08-22.** The owner-run unbriefed first-use gate remains pending and is not fabricated. Evidence: `docs/product/WP03_ACCEPTANCE_EVIDENCE.md`.

- [x] Replaced the production persistent combat hand/five-slot planner with a mandatory focused next-block PLAN offering up to two large authored locations; an explicit third-offer hook remains possible, but none is granted by current content.
- [x] Added a one-copy accessible lap deck, draw-without-replacement refill, unselected-card retention, selected-card finiteness, current/archived resolved history, and clean next-lap rebuild.
- [x] Locked the visible schema-1 draw boundary (`30301 → [gang_hideout, subway_entrance]`), `cards`-only selection, cosmetic isolation, same-seed restart, and no hidden INTRO draw.
- [x] Bound staging/confirmation to exact offer/lifecycle revisions and stable lap/block IDs; stale, malformed, replayed, wrong-phase, and transition-race intents are immutable rejections.
- [x] Preserved safe-boundary/lap-decision/boss precedence, exact-once Heat/effect authority, authored effects, Night Pressure separation, fixed-route internals, and all WP02 lifecycle contracts.
- [x] Added large icon-plus-label cards, exact block/Heat/special/reward-risk preview, native click/tap/keyboard selection, one confirm, compact Next Block, resolved history, and tutorial/help noncompetition. Production cards are not drag sources.
- [x] Removed release hand/draw/discard, supplemental card-reward, future-slot legality, route-dot, and five-target clutter while retaining isolated Milestone 5 compatibility tests.
- [x] The isolated release snapshot passed **5/5 focused tests with 130 assertions**, **72/72 affected tests with 1,388 assertions**, and **274/274 cumulative tests with 3,919 assertions across 27 suites**, with no failures or skips; configured focused dispatch also proves every authored card effect, and native GUI routing proves keyboard/touch select plus confirm.
- [x] Passed configured native `/GameRun`, three inspected 1280×720 states including a natural one-choice block-three case, Windows/Web release exports, exit-0 exported Windows runtime, and 2560×1440 production-Web real pointer prediction/occurrence/history with an empty warning/error console.
...
- [x] Added one development-only contextual Environment slot comparing preserved Hydrant, 96px/4-damage/1s-stun/3s-Shock Power Box, and one-charge 56px/65-damage Sign.
- [x] Added a development-only 3s/10s-cooldown one-press Focus priority with exact target/attack/revision/token context and no direct damage/stun/attack command.
- [x] Reused exact finite Backup behind a caller request gate; documented that isolated six-second dominance makes whole-run charge scarcity essential.
- [x] Added a separate `4 DEV` 1.1s/18s-cooldown Rally retreat to expose its survival/damage and direct-control/HUD tradeoffs; it is not promised production content.
- [x] Added deterministic non-authoritative telemetry and four exact existing-roster scenarios spanning early, middle, elite, and boss contexts.
- [x] Ran 60 configured rows across all crew, exact WP04 builds, and hold/Environment/Focus/Backup/Rally policies; exact repeat and 50-draw cosmetic isolation passed.
- [x] Passed **13/13 focused tests/180 assertions** and **304/304 cumulative tests/4,373 assertions across 32 suites** on the final run, zero failures/skips. Clean repeats reported 4,372 or 4,373 because inherited WP02/M6 branches conditionally execute one assertion; no WP05 count varied. The aggregate retains only the inherited shutdown signature.
- [x] Launched graphical configured `/GameRun`, repaired prototype obstruction/copy/clipping, and exercised exact real-pointer Focus retargeting at a captured Bottle Thrower window.
...
## WP06 implementation — World, Combat, and Presentation Polish

**Status: Implemented, technically verified, visually inspected, and owner-accepted for publication on 2026-08-30.** Evidence: `docs/product/WP06_ACCEPTANCE_EVIDENCE.md`; direction: `docs/art/wp06/WP06_ART_DIRECTION.md`.

- [x] Approved the Electric Rain Service Block direction and preserved fixed 640×360 world geometry, native 1280×720 UI, camera, combat bounds, logical lanes, and existing authorities.
- [x] Hid release-visible route/lane placeholder drawing by default while retaining development `F2` tooling.
- [x] Added deterministic Alley, Arcade, Convenience Store, Subway Entrance, and Viper profiles plus three presentation-only lap atmospheres from existing snapshots.
- [x] Added distinct silhouettes for Jax, Zoey, Rex, Street Punk, Bat Thug, Bottle Thrower, Viper Enforcer, The Viper, and Backup Runner without changing actor origins/hitboxes/timing.
- [x] Separated melee, projectile, charge, area, summon, Focus, target, and Environment shape contracts with text/non-color reinforcement.
- [x] Added bounded impact/status/major-event feedback, phase-aware existing-cue audio mixing, and input-pass-through transitions for PLAN/FIGHT/REWARD/SHOP/PUSH-EXTRACT/BOSS/RESULT.
- [x] Preserved shake-off, full hit-flash reduction, damage-numbers-off, muted-audio, focus-pause, deterministic/no-unseeded-randomness, 48-transient, and complete cleanup contracts.
- [x] Passed **15/15 WP06 focused tests/281 assertions** and the unchanged **319/319 cumulative tests/4,734 assertions**, plus a clean configured 180-frame `/GameRun` smoke.
- [x] Rendered and inspected five final 1280×720 configured captures covering District Plan, all three crew, Hydrant, Power Box, Focus, Backup, basics, elite charge, boss area, and changed world/HUD hierarchy with zero authority mutation.
- [x] Owner stated WP06 is good to go and should be published on 2026-08-30.
- [ ] Record exact implementation commit and successful Pages run after publication completes.

The local host lacks the official 4.7.2 export-template package, so a fresh WP06-local Windows export/runtime is explicitly not claimed. The Pages workflow installs official Godot Web templates and is the authoritative publication export. This disclosed platform limitation does not change the owner's publication authorization. WP06 adds no content ID, gameplay balance, save/schema/stream, Autoload, economy, progression, or compatibility-boundary change. WP07 remains unauthorized.
