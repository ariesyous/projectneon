...
The owner authorized a bounded playability repair pass on 2026-09-18 following `docs/reviews/PROJECT_STATE_2026-09-18.md`. The local repair branch synchronously invalidates projectiles when their source leaves combat and clears in-flight attacks at completed-encounter boundaries. Production telegraphs observe their exact actor/attack timeline and retire on interruption or execution; ordinary pause/resume remains lossless. Focused-plan encounter completion settles pending base coins through `RewardDirector` before decisions stop eligible time. `GameHUD` owns an input-blocking backdrop below its decision surfaces, plus concise, opaque plan/reward/shop/lap presentation. These corrections do not move authority into UI or add a content ID, stream, schema, save format, or economy.

The owner subsequently chose substantially shorter runs with the existing card rules. The configured scene now assigns `data/pacing/short_run_*.tres`: the same stable route with a 4-second approach, the same three non-boss encounter identities with a 1-second entry and 4-second reinforcement interval, and short-run cadence bands. Only Resource references and timing values change; existing controllers remain the scheduling authorities. Historical route/encounter/cadence Resources remain intact for isolated fixtures. `GameSpecifications.md` section 0.9 and `docs/reviews/SHORT_RUN_PACING_2026-09-18.md` define the revised local timing boundary; published WP06 and qualitative records are unchanged.
...
Milestone 5 was merged through PR #4 at `da934897cbdee44cb4d1a44b25e91b458558bfbc`. Milestone 6 was fast-forwarded through `a147f93`; Pages run 29960250903 deployed its tentative playtest build. WP06 was published from `224ed5c` / run 33295086207. The current [browser-playtest](https://ariesyous.github.io/projectneon/) gameplay boundary is the owner-authorized short-run repair `187556935a69feeb77dea21a4f37a0d451785fd3`; Pages run 35390747882 succeeded on 2026-09-18. Full post-deployment interaction/visual acceptance is not claimed, and WP02/WP03/WP04 qualitative records remain pending.
...
The short-run repair is published at `1875569` / Pages run 35390747882 on top of the accepted WP06 boundary `224ed5c`. Full WP07 acceptance remains unauthorized. The following remain intentionally absent and unauthorized unless a later explicit owner decision changes the specification:
