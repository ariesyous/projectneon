...
The implementation is the Milestone 6 codebase migrated through landed WP01 and WP02 scope. Never describe a later approved target as already implemented. Preserve historical M0–M6 verification, landed package contracts, and the Milestone 5 compatibility boundary while migrating prospectively through the correct owner. There is no Milestone 7; WP03–WP07 remain separate, unstarted packages.
...
WP02 Core Run Loop and State Clarity is the implemented runtime boundary. Its implementation task did not commit, push, publish, or deploy; on 2026-08-21 the owner separately authorized a `main` and GitHub Pages browser-playtest release. That release remains a playtest candidate while the five-person comprehension gate is pending and does not authorize WP03–WP07, which still require separate authorization package by package.
...
These contracts describe the preserved Milestone 6 content/runtime baseline. WP01 and WP02 subsequently migrate their owned presentation, crew-access, and district-lifecycle behavior; the focused District Plan and Focus mechanic still do not exist.
...
- WP03 has not begun. The finite Milestone 5 hand, five future route slots, placement/reward revisions/tokens, `cards` stream, and safe planning pause remain the compatibility authority until WP03 is explicitly authorized.
...
- `RunFlowController`: typed coordination between run, meaningful district blocks, exact lap-decision tokens, patrol, encounter, reward, cooling, cards, interventions, combo/cadence snapshots, boss dispatch, route-effect resolution, terminal summary, and presentation intent
...
There is no Milestone 7 or later milestone. WP00 replaces the former no-future-work statement with the approved WP01–WP07 sequence in `docs/product/ROADMAP.md`. WP01 and WP02 have landed within their recorded boundaries; WP03 is not started or implicitly authorized. Historical M6 playtest evidence remains valid and must not be relabelled as final rebaseline acceptance.
