...
The last fully accepted and published baseline is **Milestone 5 — District Cards** on top of completed **Milestone 4.2 — Inventory drag and backpack clarity correction**, **Milestone 4.1 — Equipment usability and HUD readability correction**, and **Milestone 4 — Equipment and Synergies**. The local `codex/milestone-6-vertical-slice` working branch now contains the authorized **Milestone 6 — Vertical-Slice Content and Presentation** implementation. Its cumulative automated gate passed, while representative Victory/Extraction/manual validation, cadence conformance, exports, browser-console inspection, and visual evidence are explicitly deferred TODOs. Do not describe Milestone 6 as fully accepted or published until those records are completed. The accepted Milestone 0–5 baseline preserves:
...
- **Milestone 6 (local working branch):** Godot 4.7 passed **244/244 cumulative tests and 3,234 assertions with no failures or skips across 22 suites**, preserving all 188 accepted Milestone 0–5 tests and adding 56 M6 tests. A configured headless boot opened the main scene at `/GameRun` without parser/runtime warnings, errors, or leaks. The single-process cumulative harness exited 0 but emitted 48 ObjectDB-instance and four resource-in-use shutdown diagnostics; focused/long-form harness shutdown diagnostics also remain a next-session cleanup-audit TODO because the configured production boot did not reproduce them. A fixed-seed technical Rex/starter-only probe naturally reached Defeated at 589.517 eligible seconds (9m49.5s) after reaching the boss at 584.983 seconds; it was not a manual or owner-validation pass. Representative Victory and Extraction, manual actor/boss/intervention/settings/save/restart checks, exact cadence acceptance, 1280 x 720 plus 1080p/1440p visual inspection, Windows/Web exports, exported-runtime/browser-console inspection, and fresh visual evidence are deferred TODOs.

Milestone 7 and every later test section remain unauthorized future scope. Resume only the documented Milestone 6 verification TODOs after owner review; do not expand beyond them.
