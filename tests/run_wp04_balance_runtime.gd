...
const SUITE_SCRIPT: Script = preload("res://tests/unit/wp04_balance_runtime_suite.gd")
...
func _run_suite() -> void:
	...
	print(
		"WP04_BALANCE_RUNTIME_SUMMARY=%d/%d tests, %d assertions, %d failed, %d skipped"
		% [
	...
	if int(result.get("failed", 0)) > 0 or int(result.get("skipped", 0)) > 0:
		print("WP04_BALANCE_RUNTIME_FAILURES=" + JSON.stringify(result.get("results", [])))
