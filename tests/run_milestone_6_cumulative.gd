...
## Auditable cumulative runner: all fifteen accepted Milestone 0-5 suites plus
## the seven Milestone 6 suites. Any failure or skip produces a non-zero exit.
...
func _run_cumulative_suites() -> void:
	...
	print(
		"MILESTONE_6_CUMULATIVE_SUMMARY=%d/%d tests, %d assertions, %d failed, %d skipped"
		% [
	...
	if int(result.get("failed", 0)) > 0 or int(result.get("skipped", 0)) > 0:
		...
		for test_result: Dictionary in result.get("results", []):
			if (
				not bool(test_result.get("passed", false))
				or bool(test_result.get("skipped", false))
			):
				unsuccessful_results.append(test_result)
		print("MILESTONE_6_CUMULATIVE_FAILURES=" + JSON.stringify(unsuccessful_results))
	var exit_code: int = (
		0
		if int(result.get("failed", 0)) == 0 and int(result.get("skipped", 0)) == 0
		else 1
	)
	# Release the one-process audit harness before quitting. Individual suites
	# already free every tracked runtime object; these references otherwise make
	# engine shutdown diagnostics describe the runner itself as leaked state.
	result.clear()
	suites.clear()
	runner = null
