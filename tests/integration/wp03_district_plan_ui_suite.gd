...
func test_release_plan_is_two_large_native_choices_without_legacy_route_clutter() -> void:
	...
	assert_contains(hud.district_card_instruction.text.to_upper(), "BLOCK, HEAT, AND PAYOFF", "release UI: prediction prompt is explicit")
	...
	assert_contains(hud.district_card_choice_02.tooltip_text.to_upper(), "ONE PURCHASE", "release UI: special rule shown")
