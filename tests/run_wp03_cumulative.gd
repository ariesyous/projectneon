...
## WP03 cumulative runner: the accepted Milestone 0-6 + WP01/WP02 baseline,
## then the focused District Planning and Cards authority/presentation suites.
...
func _run_cumulative_suites() -> void:
	...
	print(
		"WP03_CUMULATIVE_SUMMARY=%d/%d tests, %d assertions, %d failed, %d skipped"
		% [
	...
	if int(result.get("failed", 0)) > 0 or int(result.get("skipped", 0)) > 0:
		...
		print("WP03_CUMULATIVE_FAILURES=" + JSON.stringify(unsuccessful))
