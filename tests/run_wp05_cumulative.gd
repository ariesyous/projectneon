...
func _run() -> void:
	...
	print("WP05_CUMULATIVE_SUMMARY=%d/%d tests, %d assertions, %d failed, %d skipped" % [
		int(result.get("passed", 0)), int(result.get("total", 0)), assertions,
		int(result.get("failed", 0)), int(result.get("skipped", 0)),
	])
	print("WP05_SUITE_ASSERTIONS=" + JSON.stringify(assertions_by_suite))
	if int(result.get("failed", 0)) > 0 or int(result.get("skipped", 0)) > 0:
		...
		print("WP05_CUMULATIVE_FAILURES=" + JSON.stringify(unsuccessful))
