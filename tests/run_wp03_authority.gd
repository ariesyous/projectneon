...
const SUITE_SCRIPT: Script = preload("res://tests/unit/wp03_district_plan_suite.gd")
...
func _run_suite() -> void:
	...
	print(
		"WP03_AUTHORITY_SUMMARY=%d/%d tests, %d assertions, %d failed, %d skipped"
		% [
	...
	if int(result.get("failed", 0)) > 0 or int(result.get("skipped", 0)) > 0:
		print("WP03_AUTHORITY_FAILURES=" + JSON.stringify(result.get("results", [])))
