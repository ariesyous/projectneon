...
const SUITE_SCRIPT: Script = preload("res://tests/unit/wp02_core_run_loop_suite.gd")
...
func _run_suite() -> void:
	...
	print(
		"WP02_AUTHORITY_SUMMARY=%d/%d tests, %d assertions, %d failed, %d skipped"
		% [
	...
	if int(result.get("failed", 0)) > 0:
		print("WP02_AUTHORITY_FAILURES=" + JSON.stringify(result.get("results", [])))
