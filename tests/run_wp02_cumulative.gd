...
## WP02 cumulative runner: the accepted Milestone 0-6 + WP01 baseline, then
## authority-first Core Run Loop and State Clarity coverage.
...
func _run_cumulative_suites() -> void:
	...
	print(
		"WP02_CUMULATIVE_SUMMARY=%d/%d tests, %d assertions, %d failed, %d skipped"
		% [
	...
	if int(result.get("failed", 0)) > 0 or int(result.get("skipped", 0)) > 0:
		...
		print("WP02_CUMULATIVE_FAILURES=" + JSON.stringify(unsuccessful_results))
